...
select stad.stadium_name, stad.city, count(ms.match_id) as No_of_Matches_Played
from ipl_stadium as stad left join ipl_match_schedule as ms 
on stad.stadium_id = ms.stadium_id
group by stad.stadium_name;

#3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

select st.stadium_id, st.stadium_name, st.city,
sum(if(im.toss_winner = im.match_winner, 1, 0)) / count(st.stadium_id)*100 as P_wins_by_toss
from ipl_stadium st join ipl_match_schedule ims on 
st.stadium_id = ims.stadium_id
join ipl_match im on ims.match_id = im.match_id
where ims.status != 'cancelled'
group by st.stadium_id;

#4.	Show the total bids along with bid team and team name.
select bidding_d.bid_team, t.team_name, count(bidder_p.no_of_bids) as total_bids
from ipl_bidder_points as bidder_p left join ipl_bidding_details as bidding_d
on bidder_p.bidder_id = bidding_d.bidder_id
join ipl_team as t
on bidding_d.bid_team = t.team_id
group by bidding_d.bid_team;
...
from ipl_team t left join ipl_team_standings ts
on t.team_id = ts.team_id
group by t.team_id;

#7.	Display the bowlers for Mumbai Indians team.
select pl.player_id, pl.player_name, tpl.player_role
from ipl_player pl join ipl_team_players tpl
on pl.player_id = tpl.player_id
join ipl_team t 
on tpl.team_id = t.team_id
where tpl.player_role = 'Bowler' and t.team_id = 5;

#8.	How many all-rounders are there in each team, Display the teams with more than 4 
#all-rounder in descending order.
select remarks, count(player_role) as 'No of All Rounders'
